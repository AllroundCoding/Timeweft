...
function createShare(db, { ownerId, granteeId, preset, perms }) {
  ...
  db.prepare(`INSERT INTO timeline_shares
    (id, owner_id, grantee_id, preset, perm_timeline, perm_docs, perm_entities, perm_settings)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?)
    ...
    .run(id, ownerId, granteeId, preset || 'read',
         p.perm_timeline, p.perm_docs, p.perm_entities, p.perm_settings);
  return getShare(db, ownerId, granteeId);
}

function getShare(db, ownerId, granteeId) {
  return db.prepare('SELECT * FROM timeline_shares WHERE owner_id = ? AND grantee_id = ?')
...
function updateShare(db, ownerId, granteeId, fields) {
  ...
  if (preset && preset !== 'custom') {
    ...
    vals.push(ownerId, granteeId);
    db.prepare(`UPDATE timeline_shares SET ${sets.join(', ')} WHERE owner_id=? AND grantee_id=?`)
      .run(...vals);
  }
  return getShare(db, ownerId, granteeId);
}

function deleteShare(db, ownerId, granteeId) {
  db.prepare('DELETE FROM timeline_shares WHERE owner_id = ? AND grantee_id = ?')
